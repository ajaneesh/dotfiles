...
;; Basic font definitions. Heights are point sizes (1/10 pt); physical sizing is
;; handled session-wide by Xft.dpi (see modules/common/display.nix), which the
;; GTK Emacs build honors. So these stay fixed points and the single display.scale
;; knob controls how large they render on any display. Tune scale, not these.
